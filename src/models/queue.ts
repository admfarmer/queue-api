...
export class QueueModel {
  ...
  //Ubonket10 
  getCreate(db: knex, dateServ: any, servicePointId: any) {
    return db('q4u_queue as q')
  ...
  getWorkingRunNumber(db: knex, dateServ: any, servicePointId: any) {
    return db('q4u_queue as q')
      .select('q.service_point_id', 'q.date_serv as queue_date', 'q.room_id',
        'q.queue_number', 'q.hn', 'q.vn', 'q.queue_id', 'q.queue_interview', 'q.date_serv', 'q.time_serv', 'q.date_update', 'p.title', 'p.first_name', 'p.last_name',
        'p.birthdate', 'pr.priority_name', 'pr.priority_color',
  ...
  getHisQueueInfo(db: knex, hn: any) {
    return db('q4u_queue')
      .where('hn', hn)
      .orderBy('vn', 'desc')
      .limit(1);
  ...
  //Ubonket10 
  getPrintHisInfo(db: knex, queueId: any) {
    const sql = `
    select q.hn, q.vn, q.queue_id, q.queue_number, q.date_serv, q.time_serv,s.title, s.first_name, s.last_name,
    ...
    from q4u_queue as q 
    inner join q4u_service_points as sp on sp.service_point_id=q.service_point_id
    inner join q4u_person as s on s.hn=q.hn
    left join q4u_priorities as p on p.priority_id=q.priority_id
    where q.queue_id=?
    `;
    // console.log(sql);
  ...
  getWorkingInterview(db: knex, dateServ: any, servicePointId: any) {
    return db('q4u_queue as q')
      .select('q.service_point_id', 'q.date_serv as queue_date', 'q.room_id',
        'q.queue_number', 'q.hn', 'q.vn', 'q.queue_id', 'q.queue_interview', 'q.date_serv', 'q.time_serv', 'q.date_update', 'p.title', 'p.first_name', 'p.last_name',
        'p.birthdate', 'pr.priority_name', 'pr.priority_color',
